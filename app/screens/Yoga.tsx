    ...
    try {
      ...
      if (response.status === 201) {
        Alert.alert("Success", "Yoga activity saved successfully!", [
          { text: "OK", onPress: () => navigation.navigate("LifestyleMonitoring") }
        ]);
      } else {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          if (error.response.data.non_field_errors) {
            ...
            errorMessage = error.response.data.detail || "Failed to submit water intake.";
          }
    ...
    >
      ...
      <View style={styles.containerItem}>
        ...
        <View style={styles.textWrapper}>
          ...
          {/* Question 2 */}
          <View style={styles.row}>
            <Text style={styles.questionText}>{languageText.yogaDuration}</Text>
